...
def ang_rect(sc, x0, y0, w, h, angle, color, w1=0):
    ...
    np.sin((np.pi / 180) * angle)
    x2 = x0 - w * 0.5 * np.cos((np.pi / 180) * angle) - h * 0.5 * \
    np.sin((np.pi / 180) * angle)
    y2 = y0 + h * 0.5 * np.cos((np.pi / 180) * angle) - w * 0.5 * \
    np.sin((np.pi / 180) * angle)
    x3 = x0 - w * 0.5 * np.cos((np.pi / 180) * angle) + h * 0.5 * \
    np.sin((np.pi / 180) * angle)
    y3 = y0 - h * 0.5 * np.cos((np.pi / 180) * angle) - w * 0.5 * \
    np.sin((np.pi / 180) * angle)
    x4 = x0 + w * 0.5 * np.cos((np.pi / 180) * angle) + h * 0.5 * \
    np.sin((np.pi / 180) * angle)
    y4 = y0 - h * 0.5 * np.cos((np.pi / 180) * angle) + w * 0.5 * \
    np.sin((np.pi / 180) * angle)
    polygon(sc, color, [(int(x1), int(y1)), (int(x2), int(y2)), (int(x3),
...
def ang_rect2(sc, x0, y0, w, h, angle, quarter,
    ...
    if quarter == 3:
        bx = -1
    x0 = x0 - (bx * w * 0.5 * np.cos((np.pi / 180) * angle) - by * h * 0.5 * \
               np.sin((np.pi / 180) * angle))
    y0 = y0 - (by * h * 0.5 * np.cos((np.pi / 180) * angle) + bx * w * 0.5 * \
               np.sin((np.pi / 180) * angle))
...
def hair(r_head, r_tr, phi, n, x0, y0, sc, color_fill, color_side):
    ...
    for i in range(n):
        ...
        x1 = int(x0 + r_head * np.cos((np.pi / 180) * alpha) + \
                 np.sqrt(3) * 0.5 * r_tr * np.sin((np.pi / 180) * alpha))
        y1 = int(y0 + r_head * np.sin((np.pi / 180) * alpha) - \
                 np.sqrt(3) * 0.5 * r_tr * np.cos((np.pi / 180) * alpha))
        x2 = int(x0 + r_head * np.cos((np.pi / 180) * alpha) - \
                 np.sqrt(3) * 0.5 * r_tr * np.sin((np.pi / 180) * alpha))
        y2 = int(y0 + r_head * np.sin((np.pi / 180) * alpha) + \
                 np.sqrt(3) * 0.5 * r_tr * np.cos((np.pi / 180) * alpha))
